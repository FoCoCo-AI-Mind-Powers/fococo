// Export pages
export '/pages/login/login_widget.dart' show LoginWidget;
export '/pages/dashboard/dashboard_widget.dart' show DashboardWidget;
export '/pages/profile/profile_widget.dart' show ProfileWidget;
export '/pages/ai_insights/ai_insights_widget.dart' show AiInsightsWidget;
export '/pages/golf_rounds/golf_rounds_widget.dart' show GolfRoundsWidget;
export '/pages/coaching_modules/coaching_modules_widget.dart'
    show CoachingModulesWidget;
export '/pages/progress/progress_widget.dart' show ProgressWidget;
export '/pages/achievements/achievements_widget.dart' show AchievementsWidget;
export '/pages/register/register_widget.dart' show RegisterWidget;
export '/pages/onboarding/comprehensive_onboarding_widget.dart'
    show ComprehensiveOnboardingWidget;
